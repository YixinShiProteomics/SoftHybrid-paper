...
pacman::p_load(magrittr, ggplot2, ggVennDiagram, pheatmap, ggridges, umap, limma, dplyr, tidyr, viridis, gridExtra, stringr, ggsci, clusterProfiler, org.Mm.eg.db, UpSetR, missForest,ComplexHeatmap, WGCNA, org.Hs.eg.db, GOSemSim, ggrepel, VennDiagram, GSVA, tibble, purrr, scales, imputeLCMD, patchwork, pROC, forcats)
```
...
load_filtered_protein_matrix <- function(file_path) {
  ...
  sample_names <- unlist(lapply(strsplit(colnames(df)[7:22], split = '_', fixed = TRUE), function(x) x[10]))
  sample_names <- unlist(lapply(strsplit(sample_names, split = '.', fixed = TRUE), function(x) x[1]))
  sample_suffix <- unlist(lapply(strsplit(colnames(df)[7:22], split = '_', fixed = TRUE), function(x) x[6]))
...
#save(hybrid_results, file = './output_DIA/hybrid_results.Rdata')
load(file = './output_DIA/hybrid_results.Rdata')
```
...
```{r}
pDIA_ThreeMethods_Accuracy <- (results_plot_raw[[2]] | results_plot[[18]]) / (results_plot[[2]]|results_plot[[6]])/(results_plot[[10]] |results_plot[[14]]) +
    plot_layout(guides = "collect") +
    plot_annotation(tag_levels = 'A') &  
    theme(
    panel.border     = element_rect(colour = "black", fill = NA, linewidth = 0.6),
...
```{r}
methodwise_error_summary$Order <- c(1,2,6,3,4,7,7,9,5,11,10,12,13,14,16,15,17,18,19,20)
...
pMAE_DIA <- methodwise_error_summary %>%
  ...
  theme_bw() +
  ggtitle("DIA Global MAE across Imputation Methods and Doses") +  
  theme(
    panel.border     = element_rect(colour = "black", fill = NA, linewidth = 0.6),
    text             = element_text(size = 13),
    axis.text        = element_text(size = 13, face = "bold"),
...
pRMSE_DIA <- methodwise_error_summary %>% 
  ...
  ylab("RMSE") +
  facet_wrap(~Dose, ncol = 4, scales = "free_x") +
  ggtitle("DIA GlobalRMSE across Imputation Methods and Doses") +  
  theme(
    panel.border     = element_rect(colour = "black", fill = NA, linewidth = 0.6),
    text             = element_text(size = 13),
    axis.text        = element_text(size = 13, face = "bold"),
...
save(pMAE_DIA, file = './output/pMAE_DIA.Rdata')
...
```{r}
pMAE_DIA_10ng <- specieswise_error_summary %>% 
  filter(Dose == '10ng') %>%
...
pMAE_DIA_1ng <- specieswise_error_summary %>% 
  filter(Dose == '1ng') %>%
  ggplot(aes(x = Sample, y = MAE, fill = Method)) +
  geom_col() +
  scale_fill_manual(values = my_colors) +
...
pMAE_DIA_100pg <- specieswise_error_summary %>% 
  filter(Dose == '100pg') %>%
  ggplot(aes(x = Sample, y = MAE, fill = Method)) +
  geom_col() +
  scale_fill_manual(values = my_colors) +
...
pMAE_DIA_50pg <- specieswise_error_summary %>% 
  filter(Dose == '50pg') %>%
  ggplot(aes(x = Sample, y = MAE, fill = Method)) +
  geom_col() +
  scale_fill_manual(values = my_colors) +
...
pDIA_Species_combo <- (pMAE_DIA_50pg | pMAE_DIA_100pg) / (pMAE_DIA_1ng | pMAE_DIA_10ng) +   
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = 'A') &   
  theme(
    plot.tag = element_text(face = "bold", size = 14),
...
ggsave(
  filename = "./figure/DIA_MAE_ThreeSpecies.tiff",  
  plot = pDIA_Species_combo,                      
  device = "tiff",                    
  dpi = 300,                          
  width = 18,                         
  height = 6,                       
...
ggsave("mean_macro_numline.tiff", p_numline, width = 12, height = 9, dpi = 300)
...
ggsave("./figure/mean_macro_numline_clean.tiff", 
       p_numline, 
       width = 12, 
       height = 2, 
       dpi = 300, 
