...
pacman::p_load(magrittr, ggplot2, ggVennDiagram, pheatmap, ggridges, umap, limma, dplyr, tidyr, viridis, gridExtra, stringr, ggsci, clusterProfiler, org.Mm.eg.db, UpSetR, missForest,ComplexHeatmap, WGCNA, org.Hs.eg.db, GOSemSim, ggrepel, VennDiagram, GSVA, tibble, purrr, scales, imputeLCMD, patchwork, pROC, forcats, ggpubr)
```
...
load_filtered_protein_matrix <- function(file_path) {
  ...
  sample_names <- unlist(lapply(strsplit(colnames(df)[7:22], split = '_', fixed = TRUE), function(x) x[13]))
  sample_names <- unlist(lapply(strsplit(sample_names, split = '.', fixed = TRUE), function(x) x[1]))
  sample_suffix <- unlist(lapply(strsplit(colnames(df)[7:22], split = '_', fixed = TRUE), function(x) x[9]))
  colnames(df)[7:22] <- paste0(sample_names, "_", sample_suffix)

  df$Protein.Group <- sub(";.*", "", df$Protein.Group)
  df$Protein.Names <- sub(";.*", "", df$Protein.Names)
  df$Genes <- sub(";.*", "", df$Genes)

  filter_protein <- df$Protein.Group[!grepl("contam", df$Protein.Group)]
  
  df <- df[df$Protein.Group %in% filter_protein, ]
  df <- df[, c(2, 7:22)]
...
extract_metadata_from_colnames <- function(expr_matrix) {
  metadata <- data.frame(
    Sample = unlist(lapply(strsplit(colnames(expr_matrix), split = '_', fixed = TRUE), function(x) x[1])),
    Group  = unlist(lapply(strsplit(colnames(expr_matrix), split = '_', fixed = TRUE), function(x) x[2]))
  )
  rownames(metadata) <- metadata$Sample
  metadata$Group <- factor(metadata$Group, levels = c('SampleA', 'SampleB'))
  return(metadata)
}
...
## Read Data
```{r, warning=FALSE}
intensity_50pg <- load_filtered_protein_matrix("./data/report.pg_matrix_50pg.tsv")
intensity_100pg <- load_filtered_protein_matrix("./data/report.pg_matrix_100pg.tsv")
intensity_1ng <- load_filtered_protein_matrix("./data/report.pg_matrix_1ng.tsv")
intensity_10ng <- load_filtered_protein_matrix("./data/report.pg_matrix_10ng.tsv")
```
...
```{r}
p1 <- plot_group_missing_rate(intensity_50pg, intensity_50pg_long, metadata_50pg, label = "50pg_DIA")
p2 <- plot_group_missing_rate(intensity_100pg, intensity_100pg_long, metadata_100pg, label = "100pg_DIA")
p3 <- plot_group_missing_rate(intensity_1ng, intensity_1ng_long, metadata_1ng, label = "1ng_DIA")
p4 <- plot_group_missing_rate(intensity_10ng, intensity_10ng_long, metadata_10ng, label = "10ng_DIA")
...
ggsave(
  filename = "./figure/DIA_MissingRate_ThreeSpecies.tiff",  
  plot = pcombo,                      
  device = "tiff",                    
  dpi = 300,                          
  width = 15,                         
...
```{r}
res_50pg_DIA <- find_missing_rate_elbow(df_50pg_norm, span = 0.75,
                                        group_name = 'DIA_50pg',
                                        save_path = "./figure/DIA_50pg_elbow.tiff")
res_100pg_DIA <- find_missing_rate_elbow(df_100pg_norm, span = 0.75,
                                         group_name = 'DIA_100pg',
                                         save_path = "./figure/DIA_100pg_elbow.tiff")
res_1ng_DIA <- find_missing_rate_elbow(df_1ng_norm, span = 0.75,
                                       group_name = 'DIA_1ng',
                                       save_path = "./figure/DIA_1ng_elbow.tiff")
#res_10ng_DIA <- find_missing_rate_elbow(df_10ng_norm, span = 0.75,
#                                        group_name = 'DIA_10ng',
#                                        save_path = "./figure/DIA_10ng_elbow.tiff")
save(res_50pg_DIA, file = './output/res_50pg_DIA.Rdata')
save(res_100pg_DIA, file = './output/res_100pg_DIA.Rdata')
save(res_1ng_DIA, file = './output/res_1ng_DIA.Rdata')
#save(res_10ng_DIA, file = './output/res_10ng_DIA.Rdata')
```
...
save(data_list, file = './output_DIA/data_list.Rdata')
...
save(data_filter_list, file = './output_DIA/data_filter_list.Rdata')
```
## Imputation Method I: Minimal Probability of ALL proteins
```{r}
#minprob_results <- run_minprob(data_list)
#save(minprob_results, file = './output_DIA/minprob_results.Rdata')
...
#save(gaussian_results, file = './output_DIA/gaussian_results.Rdata')
load(file = './output_DIA/gaussian_results.Rdata')
```
## Imputation Method III: KNN of ALL proteins
```{r}
#knn_results <- run_knn(data_list)
#save(knn_results, file = './output_DIA/knn_results.Rdata')
load(file = './output_DIA/knn_results.Rdata')
```
## Imputation Method IV: Random Forest of ALL proteins
```{r}
#rf_results <- run_rf(data_list)
#save(rf_results, file = './output_DIA/rf_results.Rdata')
...
save(softhybrid_results, file = './output_DIA/softhybrid_results.Rdata')
load(file = './output_DIA/softhybrid_results.Rdata')
```
...
```{r}
save(results_list, file = './data/results_list_DIA.Rdata')
save(results_list_raw, file = './data/results_list_raw_DIA.Rdata')
```
...
for (method in impute_methods) {
  for (dose in doses) {
    title_prefix <- paste0('./plots_DIA/', method, "-", dose, '.Rdata')
...
for (dose in doses) {
    title_prefix <- paste0('./plots_DIA/', "Raw_", dose, '.Rdata')
    load(file = title_prefix)
...
```{r}
pDIA_Species_Accuracy <- (results_plot_raw[[1]] | results_plot[[1]] | results_plot[[5]] | results_plot[[9]] | results_plot[[13]] | results_plot[[17]]) / (results_plot_raw[[2]] | results_plot[[2]] | results_plot[[6]] | results_plot[[10]] | results_plot[[14]] | results_plot[[18]]) / (results_plot_raw[[3]] | results_plot[[3]] | results_plot[[7]] | results_plot[[11]] | results_plot[[15]] | results_plot[[19]]) / (results_plot_raw[[4]] | results_plot[[4]] | results_plot[[8]] | results_plot[[12]] | results_plot[[16]] | results_plot[[20]]) +   
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = 'A') &   
  theme(
    plot.tag = element_text(face = "bold", size = 14),
...
ggsave(
  filename = "./figure/DIA_Species_Accuracy.tiff", 
  plot = pDIA_Species_Accuracy,                      
  device = "tiff",                    
  dpi = 300,                         
  width = 27,                       
  height = 20,                        
...
## RF, minProb and SoftHybrid: Scatter Plot
```{r}
pDIA_ThreeMethods_Accuracy <- ((results_plot_raw[[2]] + labs(subtitle = "Missing rate = 16%"))| results_plot[[18]] | results_plot[[2]] | results_plot[[6]] | results_plot[[10]] |results_plot[[14]]) +
    plot_layout(guides = "collect") +
    plot_annotation(tag_levels = 'A') &  
    theme(
    panel.border     = element_rect(colour = "black", fill = NA, linewidth = 0.6),
...
pDIA_ThreeMethods_Accuracy

ggsave(
  filename = "./figure/DIA_ThreeMethods_Accuracy.tiff",  
  plot = pDIA_ThreeMethods_Accuracy,                      
  device = "tiff",                    
  dpi = 300,                          
  width = 22,                         
  height = 4.3,                    
...
```
## Error between imputed data and ground truth: absolute error
```{r}
...
for (name in names(results_list)) {
  ...
  df$Error <- df$Log2_Ratio - df$Truth
...
pAE_DIA_box <- ggplot(
  ...
  labs(
    x = NULL,
    y = "Absolute Error",
    title = "DIA Absolute Error across Imputation Methods"
  )

pAE_DIA_box

ggsave(
  filename = "./figure/AE_DIA_with_significance.tiff",
  plot = pAE_DIA_box,
  device = "tiff",
  dpi = 300,
  width = 8,
...
p_box <- ggplot(plot_df, aes(x = Method, y = Log2_Ratio, fill = Species)) +
  ...
  labs(
    x = NULL,
    y = "Log2 Ratio",
    title = "DIA Ratio across Doses and Methods"
  ) +
  theme_pub(base_size = 12) +
  theme(
    panel.border     = element_rect(colour = "black", fill = NA, linewidth = 0.6),
...
```{r}
ggsave(
  filename = "./figure/logratio_box_DIA.tiff",
  plot = p_box,
  device = "tiff",
  dpi = 300,
  width = 8,
...
ggplot(plot_df, aes(x = Method, y = Abs_Error, fill = Method)) +
  ...
  labs(
    x = NULL,
    title = "DIA Ratio Deviation across Methods"
  ) +
  ylim(0,6) +
  theme_pub(base_size = 12) +
  theme(
...
```{r}
ggsave(
  filename = "./figure/logratio_deviation_DIA.tiff",
  plot = p_deviation,
  device = "tiff",
  dpi = 300,
  width = 4.2,
...
evaluate_one <- function(limma_result_tbl, method_name) {
  df <- limma_result_tbl$full_result %>%
    ...
    filter(!is.na(t), !is.na(truth_dir))
  ...
  conf_long <- bind_rows(
    tibble(MethodDose = method_name, Task = "Up(yeast)",   TP=TP_up,   FP=FP_up,   FN=FN_up,   TN=TN_up),
    tibble(MethodDose = method_name, Task = "Down(ecoli)", TP=TP_down, FP=FP_down, FN=FN_down, TN=TN_down),
    tibble(MethodDose = method_name, Task = "Neutral(human)", TP=TP_neu, FP=FP_neu, FN=FN_neu, TN=TN_neu)
  ) %>%
    mutate(
...
pROC <- ggplot(roc_curves_df, aes(x = FPR, y = TPR, color = Method)) +
  ...
  coord_equal() +
  theme_bw() +
  labs(title = "ROC Curves by Method and Dose: DIA",
...
ggsave(
  filename = "./figure/macroROC_DIA.tiff",  
  plot = p_macroROC,                   
  device = "tiff",                    
  dpi = 300,                          
  width = 12,                        
...
p_macroROC_zoom <-
  ...
  theme_bw() +
  labs(
    title = "Macro ROC (Zoomed) by Method and Dose: DIA",
...
ggsave(
  filename = "./figure/macroROC_zoom_DIA.tiff",  
...
```
```{r}
ggsave("./figure/mean_macro_numline_clean_DIA.tiff", 
       p_numline, 
       width = 12, 
       height = 2, 
       dpi = 300, 
...
write.csv(
  delong_soft_vs_others,
  file = "./output_DIA/delong_soft_vs_others_DIA.csv",
  row.names = FALSE
)
```
